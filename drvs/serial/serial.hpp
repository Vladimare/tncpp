...
public:
   ...
   * все методы чтения должны возвращать доступное для считывания количество данных во  *
   *   внутренних FIFO, если в параметре buf передан 0                                  *
   * при вызове асинхронного чтения допускается передавать нулевой обработчик           *
   * асинхронный обработчик вызывается в контексте прерывания!                          */
  ...
  /* синхронная блокирующая запись, синхронная запись с таймаутом, и асинхронная запись *
   * асинхронный обработчик вызывается в контексте прерывания!                          */
   ...
   * эти вызовы также отменяют операции чтения с таймаутом (эти операции реализуются    *
  ...
  /* синхронная отправка символа в порт */
  virtual int sendchar  (unsigned char c) = 0;

  /* настройки порта, которые применяются по умолчанию */
